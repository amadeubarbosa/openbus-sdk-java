...
Também refaz a autenticação se a mesma ficar inválida por qualquer motivo, como, por exemplo, erros de rede temporários.
...
Assim, o usuário não precisa se preocupar com os vários casos de erro que possam acontecer num sistema distribuído para garantir que suas ofertas estarão sempre visíveis a outros serviços.
...
É possível ainda definir um parâmetro opcional do \assistant{} que informa o tempo de espera entre as tentativas.
...
A demo Hello Java do \assistant{} possui um \emph{jar} com as classes servidor e cliente, e pode ser executada de acordo com as instruções do arquivo README.txt que a acompanha.
...
O último passo necessário para finalizar o código do servidor é habilitar o processo a escutar as requisições CORBA que serão direcionadas para o serviço ofertado, o que pode ser feito disparando o método \emph{run} do \orb{} em uma thread separada, como demonstrado pelo código~\ref{lst:run}.
...
de retentativas que a função deve fazer em caso de erros. Esse número
...
Caso o número de retentativas se esgote, a última exceção recebida será lançada.
...
Para alterar o nível de log da API deve-se chamar o método \emph{setLevel()} enviando o nível desejado como parâmetro.
...
\begin{description}
  \item [interval] Tempo em segundos indicando o tempo mínimo de espera antes de cada nova tentativa após uma falha na execução de uma tarefa.
